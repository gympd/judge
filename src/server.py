...
def server(running: threading.Event, queue: queue.Queue[TaskInfo], host = '0.0.0.0', port = 12347):
	...
	while running.is_set():
		try:
			...
			# (version, tester_id, submit_id, user_id, task_id, language, priority) = buffer.decode('utf-8').split('\n')
			data = buffer.decode('utf-8')
			# (version, tester_id, submit_id, user_id, task_id, language, priority, magic_footer, program)
			...
			if match:
				queue.put(
					TaskInfo(
						match.group('tester_id'),
						match.group('submit_id'),
						match.group('user_id'),
						match.group('task_id'),
